...
(function (pixi_tilemap) {
	var CompositeRectTileLayer = (function (_super) {
		...
		CompositeRectTileLayer.prototype.initialize = function (
			...
			this.texPerChild =
				texPerChild ||
				pixi_tilemap.Constant.boundCountPerBuffer *
					pixi_tilemap.Constant.maxTextures;
			if (bitmaps) {
		...
		CompositeRectTileLayer.prototype.setBitmaps = function (bitmaps) {
			...
			for (i = len1; i < len2; i++) {
				var layer = new pixi_tilemap.RectTileLayer(
					this.zIndex,
		...
		CompositeRectTileLayer.prototype.addFrame = function (
			texture_,
			x,
			y,
			animX,
...
(function (pixi_tilemap) {
	var MultiTextureResource = (function (_super) {
		...
		function MultiTextureResource(options) {
			var _this =
				_super.call(this, options.bufferSize, options.bufferSize) || this;
		...
		MultiTextureResource.prototype.upload = function (
			renderer,
			texture,
			glTexture
		) {
			...
			if (doClear && !this._clearBuffer) {
				this._clearBuffer = new Uint8Array(
					pixi_tilemap.Constant.boundSize * pixi_tilemap.Constant.boundSize * 4
				);
			}
...
(function (pixi_tilemap) {
	...
	var RectTileLayer = (function (_super) {
		...
		RectTileLayer.prototype.addRect = function (
			...
			if (animCountX === void 0) {
				animCountX = 1024;
			}
			if (animCountY === void 0) {
				animCountY = 1024;
			}
		...
		RectTileLayer.prototype.renderCanvasCore = function (renderer) {
			...
			for (
				...
				i < n;
				i += pixi_tilemap.POINT_STRUCT_SIZE
			) {
				var x1 = points[i],
					y1 = points[i + 1];
				var x2 = points[i + 2],
			...
			if (this.modificationMarker !== vertices) {
				...
				for (
					...
					var u = points[i] + shiftU,
						v = points[i + 1] + shiftV;
					var rotate = points[i + 6];
					var animX = points[i + 7],
						animY = points[i + 8];
					var animWidth = points[i + 10] || 1024,
...
(function (pixi_tilemap) {
	...
	var TilemapShader = (function (_super) {
		...
		function TilemapShader(maxTextures, shaderVert, shaderFrag) {
			var _this =
				_super.call(this, new PIXI.Program(shaderVert, shaderFrag), {
		...
		function RectTileShader(maxTextures) {
			var _this =
				_super.call(
					...
					maxTextures,
					rectShaderVert,
					pixi_tilemap.shaderGenerator.generateFragmentSrc(
						maxTextures,
						rectShaderFrag
					)
				) || this;
	...
	var RectTileGeom = (function (_super) {
		...
		function RectTileGeom() {
			...
			_this
				.addAttribute('aVertexPosition', buf, 0, false, 0, _this.stride, 0)
...
(function (pixi_tilemap) {
	var TileRenderer = (function (_super) {
		...
		function TileRenderer(renderer) {
			...
			_this.texLoc = [];
			_this.texResources = [];
			_this.rectShader = new pixi_tilemap.RectTileShader(
				pixi_tilemap.Constant.maxTextures
			);
		...
		TileRenderer.prototype.initBounds = function () {
			...
			for (var i = 0; i < maxTextures; i++) {
				var resource = new pixi_tilemap.MultiTextureResource(
					pixi_tilemap.Constant
				);
		...
		TileRenderer.prototype.bindTexturesWithoutRT = function (
			renderer,
			shader,
			textures
		) {
		...
		TileRenderer.prototype.bindTextures = function (
			renderer,
			shader,
			textures
		) {
		...
		TileRenderer.prototype.checkIndexBuffer = function (size, vb) {
			...
			this.indexBuffer.update(
				PIXI.utils.createIndicesForQuads(
					size,
					pixi_tilemap.Constant.use32bitIndex
						? new Uint32Array(size * 6)
						: undefined
				)
			);
		};
...
(function (pixi_tilemap) {
	...
	(function (shaderGenerator) {
		...
		(shaderGenerator =
			pixi_tilemap.shaderGenerator || (pixi_tilemap.shaderGenerator = {}))
	);
})(pixi_tilemap || (pixi_tilemap = {}));
